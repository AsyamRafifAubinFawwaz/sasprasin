...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white shadow-lg rounded-2xl dark:bg-neutral-800 border-2 border-gray-100 dark:border-neutral-700">
            ...
            <form id="add-form" class="p-6" navigate-form action="{{ route('admin.facility-categories.do_create') }}"
                method="POST">
                @csrf

                <div class="space-y-4">
                    <div>
                        <label for="name" class="block text-sm font-medium mb-2 dark:text-white">Jenis Sarana <span
                    ...
                    <div>
                        <label for="example_items" class="block text-sm font-medium mb-2 dark:text-white">Contoh
                            Barang <span class="text-red-500">*</span></label>
                        <input type="text" id="example_items" name="example_items" value="{{ old('example_items') }}"
                            class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-orange-500 focus:ring-orange-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 placeholder-neutral-300 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 @error('example_items') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror"
                            placeholder="Contoh: CCTV" required>
                        @error('example_items')
                            <p class="text-xs text-red-600 mt-2" id="example_items-error">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="w-1/2">
                        ...
                        <div class="relative custom-select-container" id="container-priority">
                            <input type="hidden" name="priority" id="priority" value="{{ old('priority') }}" required>
                            <button type="button"
                                ...
                                <span
                                    class="selected-text">{{ old('priority') ? $priority[old('priority')] ?? 'Pilih Prioritas' : 'Pilih Prioritas' }}</span>
